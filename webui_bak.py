from GPT_SoVITS.debug.test import my_test
...
def change_tts_inference(bert_path,cnhubert_base_path,gpu_number,gpt_path,sovits_path, batched_infer_enabled):
    ...
    if batched_infer_enabled:
        ...
        # cmd = '"%s" GPT_SoVITS/debug_inference_webui.py "%s"'%('python', language)
    if(p_tts_inference==None):
        ...
        p_tts_start_time = time.perf_counter()
        p_tts_inference = Popen(cmd, shell=True)
        p_tts_end_time = time.perf_counter()
        print("change_tts_inference start sub process : %.3fs" % (p_tts_end_time-p_tts_start_time))
